...
class ReminderRepository(application: Application) {
    ...
    init {
        // These operations are safe on main thread
    ...
    suspend fun refreshReminders() = withContext(Dispatchers.IO) {
        try {
            if (userId != null) {
                val reminders = reminderDao?.getAllRemindersForUserSync(userId) ?: emptyList()
                Timber.d("Fetched ${reminders.size} reminders for user $userId")
                withContext(Dispatchers.Main) {
                    _allReminders.value = reminders
                }
            } else {
                Timber.w("Cannot refresh reminders: userId is null")
                withContext(Dispatchers.Main) {
