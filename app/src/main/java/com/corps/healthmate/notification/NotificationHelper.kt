...
class NotificationHelper(private val context: Context) {
    ...
    fun sendReminderNotification(title: String?, pills: List<String>, soundUri: Uri, isHindi: Boolean = false) {
        ...
        try {
            // Setup repeating voice reminder
            voiceHandler = Handler(Looper.getMainLooper())
            voiceRunnable = object : Runnable {
                override fun run() {
                    if (isHindi) {
                    ...
                    voiceHandler?.postDelayed(this, VOICE_REPEAT_INTERVAL)
                }
            }
            ...
            // Make notification persistent and repeating
    ...
    fun onDestroy() {
        voiceRunnable?.let { voiceHandler?.removeCallbacks(it) }
        voiceHandler = null
        voiceRunnable = null
